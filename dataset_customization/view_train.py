...
fp = bpy.path.abspath(f"//{DATASET_NAME + '/' + RESULTS_PATH}")
...
cam.location = (0, 4, 0)  # (,distance to z axis,)
cam_constraint = cam.constraints.new(type='TRACK_TO')
cam_constraint.track_axis = 'TRACK_NEGATIVE_Z'
cam_constraint.up_axis = 'UP_Y'  # cannot yaw
...
for i in range(0, VIEWS):
    ...
    frame_data = {
        'file_path': './train/r_' + str(i),
        'depth_file_path': './train/r_' + str(i) + '_depth_0001.png',
        # 'file_path': './' + scene.render.filepath.split('/')[-2] + '/' + scene.render.filepath.split('/')[-1],
...
if not DEBUG:
    with open(bpy.path.abspath(f"//{DATASET_NAME}") + '/' + 'transforms_train.json', 'w') as out_file:
